...
const HomePage = () => {
  ...
  const ProductCategory = ({ title, image, pathname }: { title: string, image: string, pathname: string }) => {
    return (
      <Link to={pathname} className="card bg-base-100 border-neutral border cursor-pointer justify-self-center hover:border-primary focus:border-primary" >
        <figure className="">
  ...
  const ServiceCategory = ({ title, image, pathname }: { title: string, image: string, pathname: string }) => {
    return (
      <Link to={pathname}>
        <button className="card bg-base-100 card-xs shadow-sm border-2 border-neutral border-square  justify-self-center cursor-pointer hover:border-primary focus:border-primary">
  ...
  */
  ...
  return (
    <div className="w-full">
      ...
      <div
        className="hero place-items-start h-[calc(100vh-64px)] bg-right contain-content bg-black"
        style={{
          backgroundImage: `url(${hero})`,
          backgroundSize: "contain",
          backgroundRepeat: "no-repeat",
      ...
      <div className="w-full p-10 mx-auto py-8 bg-base-300">
        <div className="grid grid-cols-4 gap-4">
          <ProductCategory title="Prints" image={prints} pathname="/store?category=Prints" />
          <ProductCategory title="Materials" image={materials} pathname="/store?category=Materials" />
          <ProductCategory title="Hardware" image={hardware} pathname="/store?category=Hardware" />
          <ProductCategory title="Accessories" image={accessories} pathname="/store?category=Accessories" />
        </div>
      </div>
